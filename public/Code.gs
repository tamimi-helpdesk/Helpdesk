...
function doPost(e) {
  ...
  try {
    ...
    switch (action) {
      ...
      case "cancelBooking":
        response = cancelBooking(data.bookingId || data.id, data.phoneNumber, data.reason);
        break;

      case "deleteBooking":
        response = deleteBookingFromSheet(data.bookingId || data.id, data.phoneNumber);
        break;
...
function createBooking(b) {
  ...
  for (var i = headerRowIdx + 1; i < displayData.length; i++) {
    ...
    if (rowStatus === "CONFIRMED" && rowDate === targetDateClean && rowStage === b.stage && rowStart === b.startTime) {
      return {
        success: false,
...
/**
 * Cancels a booking by Booking ID across any sheet tab
 */
function cancelBooking(bookingId, phoneNumber, reason) {
  if (!bookingId) {
    return { success: false, error: "Booking ID is required for cancellation." };
  }
  ...
  var targetRow = -1;
  var targetStatusCol = -1;
  var targetCancelledAtCol = -1;
  var targetReasonCol = -1;
  ...
  for (var s = 0; s < sheets.length; s++) {
    ...
    var cancelledAtColIdx = colMap.cancelledAt;
    var reasonColIdx = colMap.cancellationReason;

    for (var c = 0; c < (displayData[headerRowIdx] || []).length; c++) {
      var hName = String(displayData[headerRowIdx][c] || "").toLowerCase().trim();
      if (hName.indexOf("status") !== -1) statusColIdx = c;
      if (hName.indexOf("cancel") !== -1 && hName.indexOf("at") !== -1) cancelledAtColIdx = c;
      if (hName.indexOf("reason") !== -1) reasonColIdx = c;
    }
    ...
    for (var r = headerRowIdx + 1; r < displayData.length; r++) {
      ...
      if (!isMatch && cleanTargetPhone && cleanTargetPhone.length >= 7) {
        ...
        if (rowPhone && (rowPhone === cleanTargetPhone || rowPhone.indexOf(cleanTargetPhone) !== -1 || cleanTargetPhone.indexOf(rowPhone) !== -1)) {
          if (idVal.indexOf(cleanTargetId.substring(0, 4)) !== -1) {
            isMatch = true;
          }
        }
      }
      ...
      if (isMatch) {
        targetSheet = sheet;
        targetRow = r + 1;
        targetStatusCol = statusColIdx + 1;
        targetCancelledAtCol = cancelledAtColIdx + 1;
        targetReasonCol = reasonColIdx + 1;
        found = true;
        break;
      }
    }
  ...
  if (targetStatusCol > 0) {
    targetSheet.getRange(targetRow, targetStatusCol).setValue("CANCELLED");
  } else {
    var lastCol = targetSheet.getLastColumn();
    targetSheet.getRange(1, Math.max(lastCol, 12)).setValue("Status");
    targetSheet.getRange(targetRow, Math.max(lastCol, 12)).setValue("CANCELLED");
  }
  if (targetCancelledAtCol > 0) {
    targetSheet.getRange(targetRow, targetCancelledAtCol).setValue(new Date().toISOString());
  }
  if (targetReasonCol > 0) {
    targetSheet.getRange(targetRow, targetReasonCol).setValue(reason || "Cancelled via Portal");
  }
  SpreadsheetApp.flush();
  ...
  return {
    success: true,
    bookingId: bookingId,
    message: "Booking " + bookingId + " has been successfully cancelled in Google Sheets."
  };
}

/**
 * Permanently deletes a booking row by Booking ID across any sheet tab
 */
function deleteBookingFromSheet(bookingId, phoneNumber) {
  if (!bookingId) {
